...
void COverlayWndEx::OnTimer(UINT_PTR nIDEvent)
{
    if (m_bDrawing)
    {
        CRect rClient;
...
void COverlayWndEx::StartLineAnimation()
{
    m_bDrawing = true;
    SetTimer(1, 30, NULL);
}

void COverlayWndEx::StopLineAnimation()
{
    if (m_bDrawing)
    {
        m_bDrawing = false;
...
void COverlayWndEx::UpdateDrawText()
{
    ...
    //왼쪽에서 오른쪽으로 점차적으로 나타나는 선
    Pen pen(Color(255, 255, 255, 255), 5);
    graphics.DrawLine(&pen, 0, rClient.Height(), m_nLength, rClient.Height());
