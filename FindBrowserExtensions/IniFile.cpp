...
BOOL CIniFile::SetInfoType(CString strIndex, CString strName)
{
	if (!SetProfileString(strIndex, _T("browser"), strName))
	{
		return FALSE;
	}
	return TRUE;
...
CString CIniFile::GetInfoType(CString strIndex)
{
	TCHAR tName[MAX_PATH] = { _T("") };
	CString strName = _T("");

	DWORD dwResult = GetProfileString(strIndex, _T("browser"), _T(""), tName, MAX_PATH);
	strName.Format(_T("%s"), tName);
	return strName;
}
